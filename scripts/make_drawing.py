...
SLAT_INSET = 40.0
slat_x0 = led_x0 + SLAT_INSET            # 155 (slats inset for corner nuts)
...
rows = [
    ...
    ("Vange (langside)", "2", "45 x 120", "2010"),
    ("Endestykke", "2", "45 x 120", "1810"),
    ("Ben", "4", "70 x 70", f"{frame_top:.0f}"),
    ("Støtteliste", "2", f"{ledger_t:.0f} x {ledger_h:.0f}", f"{led_len:.0f}"),
    (f"Lamel (højkant)", str(n_slats), f"{slat_t:.0f} x {slat_h:.0f}", f"{inner_w:.0f}"),
]
...
notes = [
    ...
    "Støtteliste monteres med underkant i flugt",
    "med vangens underkant (bevarer frihøjden).",
...
out = "/home/dalager/projects/seng/cad/seng_tegning.svg"
...
body = [header, f'<rect x="0" y="0" width="{W}" height="{H}" fill="#ffffff"/>',
        ...
        f'Sengebund til futon 200 x 180 cm — mål i mm</text>']
body += svg
body.append('</svg>')
